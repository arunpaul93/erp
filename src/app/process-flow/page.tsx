...
export default function ProcessFlowPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [{ data: stepRows, error: stepErr }, { data: edgeRows, error: edgeErr }] = await Promise.all([
      ...
      // Filter to show only top-level nodes (no parent)
      const topLevel = (stepRows as StepRow[]).filter((r) => r.parent_step_id == null)
      const topLevelIds = new Set(topLevel.map((r) => r.id))

      const ns: Node<ProcessNodeData>[] = topLevel.map((r) => {
      ...
      // Keep only edges whose endpoints are both top-level nodes
      const es: Edge[] = (edgeRows as EdgeRow[])
        .filter((r) => r.from_step_id && r.to_step_id && topLevelIds.has(r.from_step_id!) && topLevelIds.has(r.to_step_id!))
