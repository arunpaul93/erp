...
export default function NewBudgetPage() {
  ...
  const onCreate = async () => {
    ...
    if (!periodStart) return setError('Enter a period start date')
    if (!periodEnd) return setError('Enter a period end date')

    const startDate = new Date(periodStart)
    const endDate = new Date(periodEnd)

    if (startDate >= endDate) {
      return setError('Period end date must be after the start date')
    }
        ...
        <div className="px-4 sm:px-0">
          ...
          <div className="mt-4 rounded-xl border border-gray-800 bg-gray-900/80 p-4">
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-sm text-gray-300 mb-1">Budget Name</label>
                <input
                  value={name}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm text-gray-300 mb-1">Period Start Date</label>
                  <input
                    type="date"
                    value={periodStart}
              ...
              <div className="flex gap-2">
                <button
                  onClick={onCreate}
                  disabled={loading}
                  className="bg-yellow-400 hover:bg-yellow-500 disabled:opacity-50 text-gray-900 px-4 py-2 rounded-md"
                >
                  {loading ? 'Creating...' : 'Create Budget'}
                </button>
                <button
                  onClick={() => router.push(`/business-plan/${planId}/budgets`)}
